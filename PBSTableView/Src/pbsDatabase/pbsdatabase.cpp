...
PBSDatabase::PBSDatabase(const QString connectName, const QString driver, const QString dbName, const QString userName,
                         QString password, const QString hostName, const int port)
    : QObject(NULL),
...
bool PBSDatabase::open()
{
    ...
    if (!e.isValid() && e.type() != QSqlError::NoError)
    {
        pbsShowLastDBError(NULL, pDatabase);
        return false;
    }
